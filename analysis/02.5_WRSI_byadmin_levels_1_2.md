...
```python
def aggregating_admin(admlvl, geodf, admID1, admID2, mask):
    if mask:
        clipped_path = "_clipped"
    else:
        clipped_path = ""
    for rfe in rfe_sources:
        ...
        for area in areas:
            ...
            for file in file_list:
                ...
                adm_wrsi_df = pd.concat([adm_wrsi_df, file_df], axis=0)
        adm_wrsi_df.to_csv(
            wrsi_dir / str("ner_" + admlvl + clipped_path + "_" + rfe + "_wrsi.csv"),
            index=False,
        )
```
...
```python
adm0_wrsi_df = aggregating_admin(
    admlvl = "adm0", geodf=ner_shp_adm0, admID1="ISO3", admID2="NOMPAY", mask = False
)
```

```python
adm0_wrsi_df = aggregating_admin(
    admlvl = "adm0", geodf=ner_shp_adm0_clipped, admID1="ISO3", admID2="NOMPAY", mask = True
)
```
...
```python
aggregating_admin(
    admlvl = "adm1", geodf=ner_shp_adm1, admID1="rowcacode1", admID2="NOMREG", mask = False
)
```
...
```python
aggregating_admin(
    admlvl = "adm1", geodf=ner_shp_adm1_clipped, admID1="rowcacode1", admID2="NOMREG", mask = True
)
```
...
```python
aggregating_admin(
    admlvl = "adm2", geodf=ner_shp_adm2, admID1="rowcacode2", admID2="NOMDEP", mask = False
)
```
...
```python
aggregating_admin(
    admlvl = "adm2", geodf=ner_shp_adm2, admID1="rowcacode2", admID2="NOMDEP", mask = True
)
```
